...
shaka.sync.SyncManager = class extends shaka.util.FakeEventTarget {
   ...
   */
  connect(roomId) {
    // Get PubNub constructor - prefer config, fall back to global
    const PubNubClass = this.config_.PubNub ||
        (typeof PubNub !== 'undefined' ? PubNub : null);

    // Validate that PubNub is available
    if (!PubNubClass) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.PLAYER,
          shaka.util.Error.Code.SYNC_PUBNUB_NOT_LOADED,
          'PubNub SDK not found. Pass it via config.PubNub, ' +
          'or include it via script tag, or install via npm and ' +
          'set window.PubNub = PubNub.');
    }
 ...
 *   PubNub: (function(new:PubNub, !Object)|undefined)
 * }}
 *
 ...
 *   a random ID will be generated. Note: Each client in a sync session
 *   MUST have a unique userId to receive messages from other clients.
 ...
 * @property {function(new:PubNub, !Object)|undefined} PubNub
 *   Optional PubNub constructor class. If not provided, the SyncManager
 *   will look for a global `PubNub` variable. This allows you to pass
 *   the PubNub class directly when using npm/bundlers:
 *   ```
